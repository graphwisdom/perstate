...
| `perstate-view.sh` | Browser rendering: interactive HTML graph (sigma.js v3 WebGL + graphology/forceatlas2 via esm.sh). Light theme, nodes colored by entity type, thin pale edges, hover pill, neighbor-highlight on select. Renders full graph (no node cap). awk-batch JSON extraction. | `--session-id <id>` `[--output <path>]` |
