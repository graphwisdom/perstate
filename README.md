...
| `perstate-view.sh` | Browser rendering: interactive HTML graph (sigma.js v3 WebGL + graphology/forceatlas2 via esm.sh). Dark-themed nodes by type, curved edges colored by relation type, hover pill, neighbor-highlight on select. Renders full graph (no node cap). awk-batch JSON extraction. | `--session-id <id>` `[--output <path>]` |
