...
**view**: generates interactive HTML graph (sigma.js v3 WebGL + graphology/forceatlas2 via esm.sh), auto-opens browser. Renders the full graph (no 1000-node cap). Nodes colored by entity type, curved edges colored by relation type, hover pill, neighbor-highlight on select. Script internally calls prepare.sh.
