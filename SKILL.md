...
**view**: generates interactive HTML graph (sigma.js v3 WebGL + graphology/forceatlas2 via esm.sh), auto-opens browser. Renders the full graph (no 1000-node cap). Light theme, nodes colored by entity type, thin pale edges, hover pill, neighbor-highlight on select. Script internally calls prepare.sh.
